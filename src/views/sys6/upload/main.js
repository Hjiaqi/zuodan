    ...
    for (const key in item) {
      ...
      if (key === "产品规格") {
        ...
        if (val.indexOf("Size") === -1) {
          ...
          obj["worldCupPatch"] = _setCnPatch(strObj['worldCupPatch'])
          obj["worldCupPatch2"] = _setCnPatch(strObj['worldCupPatch2'])
          //重要，只有补丁，没有定制信息和号码，也算是定制
          // 所以要加一个判断
          if(obj["spec"] === 'NO' || obj["spec"] === 'No'  || obj["spec"] === 'no' ||obj["spec"] === ''){
            // 世界杯补丁
            const haveWorldCupPatch = _setCnPatch([strObj['worldCupPatch'], strObj['worldCupPatch2']]) && strObj['worldCupPatch'] !== 'No Patch' && strObj['worldCupPatch2'] !== 'No Patch';
            if(obj["_customPatch"] || haveWorldCupPatch){
