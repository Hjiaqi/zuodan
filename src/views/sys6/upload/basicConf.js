...
function extractSpecs(specString, keywords) {
  ...
  // 1. 收集每个关键词第一次出现的位置（兼容半角/全角冒号，以及冒号前可能有空格）
  const occurrences = [];
  for (const kw of keywords) {
    // 搜索各种可能的冒号前缀组合
    const patterns = [
      { pattern: kw + ':', colonLen: 1 },     // 无空格 + 半角冒号
      { pattern: kw + ' :', colonLen: 1 },    // 1空格 + 半角冒号
      { pattern: kw + '  :', colonLen: 1 },   // 2空格 + 半角冒号
      { pattern: kw + '：', colonLen: 1 },    // 无空格 + 全角冒号
      { pattern: kw + ' ：', colonLen: 1 },   // 1空格 + 全角冒号
      { pattern: kw + '  ：', colonLen: 1 },  // 2空格 + 全角冒号
    ];

    let foundIdx = -1;
    let foundPattern = null;
    for (const p of patterns) {
      const idx = specString.indexOf(p.pattern);
    ...
    if (foundIdx !== -1) {
      // 计算冒号前有多少个空格
      const spaceCount = foundPattern.pattern.length - kw.length - 1; // 去掉关键词和冒号本身的长度
      occurrences.push({
        keyword: kw,
        start: foundIdx,
        end: foundIdx + kw.length + spaceCount + 1  // 跳过关键词 + 空格 + 冒号
      });
    }
  }

  // 2. 按关键词长度降序，优先保留更长的
  occurrences.sort((a, b) => b.keyword.length - a.keyword.length);
  ...
  for (const item of occurrences) {
    ...
    for (const sel of selected) {
      if (item.start < sel.end && item.end > sel.start) {
        overlap = true;
        break;
      }
    }
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    const valueEnd = next ? next.start : specString.length;
    let value = specString.substring(valueStart, valueEnd).trim();
    ...
    // 去掉 _mimikyu_price_ 及后面的内容（这是嵌入在值里的价格后缀，不是分隔符）
    const priceSuffixIndex = value.indexOf('_mimikyu_price_');
    if (priceSuffixIndex !== -1) {
      value = value.substring(0, priceSuffixIndex).trim();
    }

    // 如果值为空则赋空字符串
    result[toCamelCase(cur.keyword)] = value || '';
  }
  console.log('查看切割后的数据======', result)
  return result;
}
...
export const _splitChangPingGuiGe = (str) => {
  ...
  const testdata = extractSpecs(str, keywords)
  console.log('格式化函数输出======', keywords, testdata, resultObj)
...
export const _setCnPatch = (str) => {
  ...
  const convert = (item) => {
    if (!item || item === 'NO' || item === 'No' || item === 'no') {
      return '';
    }

    if (__win_data.isPathUseEN) {
      const inx = _findIndex(_patchOpts, ['value', item]);
      return inx === -1 ? item : _patchOpts[inx].label;
    }
  ...
  if (Array.isArray(str)) {
    const res = str.map(convert);
    // 如果所有项都为空，则返回 null
    return res.every(item => item === '') ? null : res;
  }
